...
// Formulario popup para registrar un producto nuevo.
// Los campos obligatorios son Código y Nombre. El resto (imagen, descripción, vencimiento) son opcionales.

// Popup para crear un producto nuevo en el inventario.
// Se abre desde el panel de Productos con el botón Nuevo.
public class pRegistrarProducto extends JPanel {
    ...
    private JTextField    txtCodigo, txtNombre, txtPrecio, txtStock, txtFechaVenc;
    private JTextArea     txtDescripcion;
    private JComboBox<String> cbCategoria;
    private JLabel        lblImagenPreview;
    private String        rutaImagenFinal;

    public pRegistrarProducto() { init(); }
    ...
    private void init() {
        ...
        lblImagenPreview.setFont(lblImagenPreview.getFont().deriveFont(11f));
        ...
        add(lbl("Código *"));         add(campo(txtCodigo    = new JTextField(), "Ej: P-001"));
        add(lbl("Nombre *"));         add(campo(txtNombre    = new JTextField(), "Nombre del producto"));
        add(lbl("Precio ($) *"));     add(campo(txtPrecio    = new JTextField(), "Ej: 9.99"));
        add(lbl("Stock inicial *"));  add(campo(txtStock     = new JTextField(), "Ej: 100"));
        add(lbl("Categoría"));
        cbCategoria = new JComboBox<>(CATEGORIAS);
        cbCategoria.putClientProperty(FlatClientProperties.STYLE, "arc:15;");
        add(cbCategoria, "growx");

        add(lbl("Fecha Vencimiento"));
        add(campo(txtFechaVenc = new JTextField(), "YYYY-MM-DD (opcional)"));
        ...
        //"arc:1borderWidth:0; margin:8,10,8,10;");
    ...
    // Devuelve true si todo salió bien (para que el popup se cierre).
    public boolean guardarDatos() {
        ...
        try {
            ...
            if (precio < 0 || stock < 0) {
                raven.alerts.MessageAlerts.getInstance().showMessage("Valor inválido", "El precio y el stock no pueden ser negativos."); return false;
            }
            ...
            if (new dao.ProductoDAO().registrar(p)) {
                raven.toast.Notifications.getInstance().show(
                    raven.toast.Notifications.Type.SUCCESS,
                    raven.toast.Notifications.Location.BOTTOM_CENTER, "Producto guardado");
                return true;
            }
            raven.alerts.MessageAlerts.getInstance().showMessage("Error al guardar", "No se pudo guardar. Verifica que el código no esté duplicado.");
